...
with open('parameters_fillsingle.yaml', 'r', encoding='utf-8') as f:
...
for isub in range(0, n_nl * n_ns):
    ...
    for i_img in range(0, n_image):
        ...
        # # 检查原始数组的形状
        # print("Original shape:", imagei_new.shape)

        # # 计算可能的新形状
        # num_elements = imagei_new.size  # 获取元素总数
        # new_shape = (num_elements // 6, 6)  # 计算新的形状

        # # 尝试重塑数组
        # try:
        #     new_imagei = imagei_new.reshape(new_shape)
        #     print("New shape:", new_imagei.shape)
        # except ValueError as e:
        #     print("Error:", e)
        new_shape = (imagei_new.shape[0] * imagei_new.shape[1], 1)
        new_imagei = imagei_new[:, :, :1].reshape(new_shape)
